                                ...
                                >
                                    <span className={`text-base`}>
                                        ...
                                        >
                                            <div
                                                onClick={(x) => {
                                                    window.open(file.url.toString(), '_blank')
                                                }}
                                                className={`col-span-3 cursor-pointer`}
                                            >
                                                <>
                                                    {fileTypes.some((fileType) => file.file.includes(fileType)) ? (<>
                                                            {['pdf'].some((extension) => file.file
                                                                .toString()
                                                                .includes(extension)) && <PDFIcon/>}
                                                            {['docx', 'doc', 'rtf', 'odt',].some((extension) => file.file
                                                                .toString()
                                                                .includes(extension)) && <DOCXIcon/>}
                                                            {['pptx'].some((extension) => file.file
                                                                .toString()
                                                                .includes(extension)) && <PPTXIcon/>}
                                                            {['xls', 'xls', '.xlsx',].some((extension) => file.file
                                                                .toString()
                                                                .includes(extension)) && <XLSXIcon/>}
                                                            {['mp4', 'mkv', 'mov', 'avi', 'wmv', 'flv', 'm4v', 'm4a', 'm4b', 'm4p', 'm4r', 'm4v',].some((extension) => file.file
                                                                .toString()
                                                                .includes(extension)) && <VideoIcon/>}
                                                            {['jpg', 'png', 'jpg', 'jpeg', 'gif', 'bmp', 'tiff', 'tif', 'psd', 'raw', 'webp', 'svg',].some((extension) => file.file
                                                                .toString()
                                                                .includes(extension)) && <ImageIcon/>}
                                                            {['mp3'].some((extension) => file.file
                                                                .toString()
                                                                .includes(extension)) && (<AudioFileIcon/>)}
                                                            {['txt'].some((extension) => file.file
                                                                .toString()
                                                                .includes(extension)) && <TXTIcon/>}
                                                        </>) : (<>
                                                            <OtherFileIcon/>
                                                        </>)}
                                                </>
                                            ...
                                            >
                                                {file.file
                                                    ...
                                                    .slice(file.file
                                                        .toString()
                                                        .indexOf('-') + 1, file.file.toString().length)}
                                            </div>
                                        </div>
                                    </span>
                                    <div className={`relative bottom-1 left-2`}>
                                        <RiArrowDropDownLine
                                            className={`${open ? 'rotate-180 transform' : ''} h-7 w-7 text-gray-600`}
                                        />
                                    </div>
                                </Disclosure.Button>
                                <Disclosure.Panel
