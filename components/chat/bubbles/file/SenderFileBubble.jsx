                            ...
                            <>
                                <Disclosure.Button
                                    ...
                                    <span className={`text-base`}>
                                        <div className={`grid grid-cols-10 content-around`}>
                                            <div
                                                onClick={(x) => {
                                                ...
                                                <>{fileTypes.some(fileType => file.file.includes(fileType))
                                                    ? <>
                                                        {file.file.toString().includes('.pdf') && <PDFIcon/>}
                                                        {file.file.toString().includes('.docx') && <DOCXIcon/>}
                                                        {file.file.toString().includes('.pptx') && <PPTXIcon/>}
                                                        {file.file.toString().includes('.xlsx') && <XLSXIcon/>}
                                                        {file.file.toString().includes('.mp4' || 'mkv' || 'mov') &&
                                                            <VideoIcon/>}
                                                        {file.file.toString().includes('.png') && <ImageIcon/>}
                                                        {file.file.toString().includes('.mp3') && <AudioFileIcon/>}
                                                        {file.file.toString().includes('.txt') && <TXTIcon/>}
                                                    </>
                                                    :
                                                    <>
                                                        <OtherFileIcon/>
                                            ...
                                            <div className={`col-span-7 my-auto`}>{file.file.substring(0, 20)}</div>
                                        </div>
                                    </span>
