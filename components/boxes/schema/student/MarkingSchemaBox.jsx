...
const MarkingSchemaBox = () => {
    const [markingSchema, setMarkingSchema] = useState(markingSchemaFileStaticList);

    const pdf = <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 48 48" width="48px" height="48px">
        <path fill="#e53935"
    ...
    const docx = <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 48 48" width="48px" height="48px">
        <linearGradient id="Q7XamDf1hnh~bz~vAO7C6a" x1="28" x2="28" y1="14.966" y2="6.45"
        ...
        <path fill="url(#Q7XamDf1hnh~bz~vAO7C6b)"
              d="M12,33.054V40c0,1.105,0.895,2,2,2h28c1.105,0,2-0.895,2-2v-6.946H12z"/>
        <linearGradient id="Q7XamDf1hnh~bz~vAO7C6c" x1="28" x2="28" y1="-15.46" y2="-15.521"
                        gradientUnits="userSpaceOnUse">
        ...
        <linearGradient id="Q7XamDf1hnh~bz~vAO7C6d" x1="12" x2="44" y1="28.53" y2="28.53"
                        gradientUnits="userSpaceOnUse">
        ...
        <path
            ...
            opacity=".07"/>
        <path d="M22.106,35H12V13.667h10.106c1.414,0,2.56,1.146,2.56,2.56V32.44C24.667,33.854,23.52,35,22.106,35z"
              opacity=".09"/>
        <linearGradient id="Q7XamDf1hnh~bz~vAO7C6e" x1="4.744" x2="23.494" y1="14.744" y2="33.493"
                        gradientUnits="userSpaceOnUse">
            <stop offset="0" stopColor="#256ac2"/>
            <stop offset="1" stopColor="#1247ad"/>
        </linearGradient>
        <path fill="url(#Q7XamDf1hnh~bz~vAO7C6e)"
    ...
    const video = <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 48 48" width="48px" height="48px">
        <radialGradient id="zzRUl~RK3zhi1YlAgR3VLa" cx="43.956" cy="41.272" r="48.447" gradientUnits="userSpaceOnUse">
        ...
        <path fill="url(#zzRUl~RK3zhi1YlAgR3VLa)"
              d="M42,7H6C4.895,7,4,7.895,4,9v30c0,1.105,0.895,2,2,2h36c1.105,0,2-0.895,2-2V9	C44,7.895,43.105,7,42,7z"/>
        <radialGradient id="zzRUl~RK3zhi1YlAgR3VLb" cx="8.431" cy="17.365" r="36.063" gradientUnits="userSpaceOnUse">
            <stop offset="0" stopColor="#4b4b4b"/>
            <stop offset=".531" stopColor="#393939"/>
            <stop offset="1" stopColor="#252525"/>
        </radialGradient>
        <rect width="34" height="25" x="7" y="13" fill="url(#zzRUl~RK3zhi1YlAgR3VLb)"/>
    ...
    const txt = <img src="https://img.icons8.com/fluency/96/000000/notepad.png" alt={"text icon"}
                     className={"w-14 h-14"}/>;

    const pptx = <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 48 48" width="48px" height="48px">
    ...
    const xlsx = <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 48 48" width="48px" height="48px">
        ...
        <path d="M22.106,35H12V13.667h10.106c1.414,0,2.56,1.146,2.56,2.56V32.44C24.667,33.854,23.52,35,22.106,35z"
              opacity=".09"/>
        <linearGradient id="flEJnwg7q~uKUdkX0KCyBa" x1="4.725" x2="23.055" y1="14.725" y2="33.055"
                        gradientUnits="userSpaceOnUse">
    ...
    return (
        <div>
            <DownloadFileWrapper topicName={"Marking Schema"} btnName={"Download Marking Schema"}>
                <div className={"flex flex-row gap-5 my-40 py-10"}>
                    {markingSchema.map((schema,index) => (
                        <div key={index} className={"grid grid-cols-6 items-center pl-3 gap-4 bg-gray-300 shadow rounded-lg"}>
                            {(schema.fileType === 'pdf') && <>
                                {pdf}
                            </>}{(schema.fileType === 'docx' || schema.fileType === 'doc') && <>
                            {docx}
                        </>}{(schema.fileType === 'mp4') && <>
                            {video}
                        </>}{(schema.fileType === 'txt') && <>
                            {txt}
                        </>}{(schema.fileType === 'pptx' || schema.fileType === 'ppt') && <>
                            {pptx}
                        </>}{(schema.fileType === 'xlsx' || schema.fileType === 'xls') && <>
                            {xlsx}
                        </>}
                            <div className={"col-span-5 px-2 py-2.5 flex flex-col gap-3 bg-gray-100"}>
                                <div className={"grid grid-cols-8 items-center"}>
                                    <div className={"col-start-1 col-end-8 capitalize text-lg font-semibold"}>{schema.fileName}</div>
                                    <MdDownload className={"col-start-8 col-end-9 w-5 h-5"}/>
                                </div>
                                <div className={"grid grid-cols-8 items-center"}>
                                    <div className={"col-start-1 col-end-8 text-sm text-gray-500"}>{schema.updatedAt}</div>
                                    <div className={"col-start-8 col-end-9 justify-start text-xs text-gray-500"}>{schema.fileSize}</div>
                                </div>
                            </div>
                        </div>
                    ))}
...
export default MarkingSchemaBox;