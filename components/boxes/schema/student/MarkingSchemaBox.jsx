...
const MarkingSchemaBox = () => {
    ...
    const video = (
        ...
        >
            ...
            <polygon
                fill="#af4e63"
                points="39,7 35,7 37,10 35,13 39,13 41,10"
            />
            <polygon
                fill="#af4e63"
                points="30,7 26,7 28,10 26,13 30,13 32,10"
            />
            <polygon
                fill="#af4e63"
                points="21,7 17,7 19,10 17,13 21,13 23,10"
            />
    ...
    return (
        <div>
            <DownloadFileWrapper
                topicName={'Marking Schema'}
                btnName={'Download Marking Schema'}
            >
                <div className={'flex flex-row gap-5 my-40 py-10'}>
                    {markingSchema.map((schema, index) => (
                        ...
                        >
                            {schema.fileType === 'pdf' && <>{pdf}</>}
                            {(schema.fileType === 'docx' ||
                                schema.fileType === 'doc') && <>{docx}</>}
                            {schema.fileType === 'mp4' && <>{video}</>}
                            {schema.fileType === 'txt' && <>{txt}</>}
                            {(schema.fileType === 'pptx' ||
                                schema.fileType === 'ppt') && <>{pptx}</>}
                            {(schema.fileType === 'xlsx' ||
                                schema.fileType === 'xls') && <>{xlsx}</>}
                            <div
                                className={
